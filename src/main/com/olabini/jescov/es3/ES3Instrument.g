...
@parser::header {
  package com.olabini.jescov.es3;
...
@after {
	...
	: logicalORExpression ( QUE thenAssignment=assignmentExpression COLON elseAssignment=assignmentExpression )?
	  -> {$thenAssignment.text != null && (instrumented=true)}? instrument_qif(conditional = {$logicalORExpression.text}, left = {$thenAssignment.text}, right = {$elseAssignment.text}, hash = {$program::hash}, bid = {bid})
	  -> pass(stmt={$text})
	;
...
@after {
	...
	: logicalORExpressionNoIn ( QUE thenAssignment=assignmentExpressionNoIn COLON elseAssignment=assignmentExpressionNoIn )?
	  -> {$thenAssignment.text != null && (instrumented=true)}? instrument_qif(conditional = {$logicalORExpressionNoIn.text}, left = {$thenAssignment.text}, right = {$elseAssignment.text}, hash = {$program::hash}, bid = {bid})
	  -> pass(stmt={$text})
	;
